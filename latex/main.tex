...
%	DIPLOMA INFO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\ttitle}{Inženiring pozivov pri razvoju programske opreme}
...
\noindent\textbf{Naslov:} Inženiring pozivov pri razvoju
programske opreme\\
...
\bigskip
\noindent\textbf{Opis:}\\
Hiter razvoj umetne inteligence, zlasti velikih jezikovnih modelov, je spremenil razvoj programske opreme. Ti modeli namreč omogočajo avtomatizirano generiranje in dopolnjevanje kode s pomočjo inženiringa pozivov. V diplomski nalogi preglejte pogosto uporabljene modele in in na izbranem izvedite eksperiment. Pri tem ovrednotite vpliv starosti in predznanja sodelujočih, težavnosti izziva ter uporabo velikih jezikovnih modelov na uspešnost reševanja nalog programskega inženirstva.
...
Predlagani so bili različni modeli življenjskega cikla, ki temeljijo na nalogah, vključenih v razvoj in vzdrževanje programske opreme.
...
V tem delu se bomo osredotočili na najpopularnejše generativne modele ChatGPT \cite{openai_chatgpt}, GitHub Copilot \cite{github_copilot} in Amazon Codewhisperer \cite{saasworthy_codewhisperer}. \\
...
V delu se bomo osredotočili na analizo možne izboljšave pri fazi implementacije in generiranja kode, ki v življenjskem ciklu programske opreme porabi največ časa in znanja. Delna avtomatizacija tega dela cikla bi v veliki meri pospešila razvoj programske opreme ter zmanjšala stroške pri njenem razvoju. Pogledali bomo inženiring pozivov ter analizirali njegov vpliv na učinkovitost implementacije s primerjavo pisanja kode s pomočjo orodja UI ChatGPT v primerjavi s tradicionalnim načinom, torej s pisanjem kode brez asistence. 
...
V nadaljevanju bomo v poglavju \ref{sec:LLM} proučili velike jezikovne modele ter njihove najpopularnejše predstavnike pri inženiringu pozivov. V poglavju \ref{sec:chatGPT} bomo obravnavali model ChatGPT, ki je svetovno najpopularnejši klepetalni robot, ki je poleg pogovora zmožen generiranja programske kode\cite{wu2023brief}. \\
V poglavjih \ref{sec:Copilot} in \ref{sec:codewhisperer} bomo proučili najbolj pogosto uporabljena asistenta za kodiranje, ki se ju uporablja znotraj integriranih razvojnih okolij, GitHub Copilot in Amazon CodeWhisperer. \\
V poglavju \ref{chap:primerjava} bomo nato predhodno omenjene modele medsebojno primerjali glede na več kriterijev, vključno z njihovo učinkovitostjo pri asistenci pri kodiranju, s podprtimi programskimi jeziki in z dostopnostjo. 

\chapter{Inženiring pozivov}

Inženiring pozivov je razmeroma nova tehnika na področju obdelave naravnega jezika, ki zajema načrtovanje in optimizacijo pozivov, ki jih uporabljamo za vnos informacij v modele UI, da bi izboljšali njihovo učinkovitost pri določenih nalogah. \cite{wang2024prompt} \\
Gre za prakso učinkovitega sodelovanja s sistemi UI za optimizacijo njihovih koristi. Z nedavnim napredkom velikih jezikovnih modelov se je inženirstvo pozivov izkazalo za pomembno in učinkovito na različnih področjih, vse od področja programiranja in učenja do področja financ in zdravstva. \cite{info:doi/10.2196/50638} \\

\section{Veliki jezikovni modeli}\label{sec:LLM}
...
Napredek na področju UI, zlasti na področju velikih jezikovnih modelov, je močno vplival na številna druga področja, vključno z izobraževanjem in razvojem programske opreme. ChatGPT je postal pomembno orodje tako v izobraževalnih okoljih kot tudi v praktičnih programerskih aplikacijah. To poglavje obravnava vlogo ChatGPT v inženiringu pozivov in razvoju programske opreme, ocenjuje njegove prednosti, izzive in potencialne vplive v prihodnosti. \\
...
\section{GitHub Copilot}\label{sec:Copilot}
...
\section{Amazon CodeWhisperer}\label{sec:codewhisperer}
...
\pagebreak
\section{Ugotovitve in primerjava}\label{chap:primerjava}
...
Ekipa OpenAI je sestavila 164 nalog s testi, da bi ocenila, ali
...
Namen diplomskega dela je bil raziskati, kako inženiring pozivov vpliva na proces razvoja programske opreme, s preučevanjem kakovosti in pravilnosti kode pri uporabi orodij umetne inteligence. Najprimernejša metoda za pridobivanje teh podatkov je bil nadzorovan eksperiment na skupini mlajših otrok, ker omogoča natančen nadzor nad spremenljivkami ter izolirano vrednotenje reševanja programskih problemov. Ta metodologija minimizira vplive zunanjih spremenljivk, kar omogoča osredotočeno proučevanje zmožnosti in omejitev inženiringa pozivov pri razvoju programske opreme.
...
\begin{figure}[H]
    ...
    \caption{Pogostost uporabe umetne inteligence}
    \label{fig:ai_ocena}
\end{figure}
...
\section{Ocenjevanje}
Po končanem eksperimentu sem naloge ročno ocenil na lestvici 1-5 glede na izhodne podatke oddanih nalog. Kakovost in berljivost kode se zaradi lažjega in objektivnejšega ocenjevanja pri oceni ni upoštevala, ocenjevalo se je pravilnost izhoda za določeno nalogo ter upoštevanje navodil in robnih primerov. \\
Oceno ena in dva so dobile naloge, ki niso bile pravilno rešene ter za noben primer vhoda ne vrnejo pravilnega izhoda. Naloge so dobile oceno tri, če so bile delno pravilno rešene, torej delujejo v nekaterih primerih oziroma navodila niso bila v celoti implementirana. Ocena štiri je bila določena nalogam, ki za osnovno podan vhod delujejo pravilno, ne delujejo pa za vse možne vhode. Ocena pet je bila izbrana za rešitve, ki ob pričakovanih vhodih vrnejo tudi pričakovan izhod. Dovoljena je bila uporaba funkcij iz vgrajenih Pythonovih knjižnic.  Skupno je bilo oddanih 196 veljavnih nalog s kodo. V povprečju so učenci zaradi časovne omejitve rešili med šest in sedem nalog od skupno osmih nalog. 
...
Zaradi visokih rezultatov pri 6. in 8. nalogi, kjer je bila omogočena uporaba ChatGPT-ja, sta povprečji druge in tretje kategorije višji od prve.
...
Pri nalogah prve stopnje težavnosti je bilo z uporabo GPT povprečno število doseženih točk 3,5, brez možnosti uporabe pa 3,2, z razliko 0,3 točke ali 6~\%. Največjo razliko lahko opazimo pri nalogah druge in tretje stopnje težavnosti, kjer so naloge z možnostjo uporabe ChatGPT bile rešene za 32~\% boljše. Pri nalogah druge stopnje težavnosti je z uporabo GPT povprečje točk 4,5 ter brez 2,8. Razlika znaša 1,7 točke ali 34~\%. Pri nalogah tretje stopnje je povprečje z uporabo GPT 4,2 ter brez 2,7, razlika je 1,5 točke ali 30~\%. Če primerjamo rezultate nalog, pri katerih GPT ni bil omogočen, lahko opazimo, da so bile v vseh primerih naloge, kjer je bila omogočena uporaba GPT, rešene boljše, v povprečju za 23,3~\%. \\
...
\end{table}
Pri nalogah prve kategorije, pri katerih je bil na voljo ChatGPT, je bil ChatGPT uporabljen povprečno le 1,9-krat na nalogo za posamezno osebo, pri nalogah druge kategorije je bil uporabljen 4,2-krat ter pri nalogah tretje kategorije 3,9-krat. Majhno število uporabe pri nalogah prve kategorije nakazuje na dejstvo, da so naloge bile lažje in se jih je zato v večji meri reševalo samostojno oziroma z minimalno asistenco kljub možnosti uporabe ChatGPT-ja. Pri težjih nalogah je pogosteje prišlo do neznanja in takrat je bil uporabljen ChatGPT. Iz analize postavljenih vprašanj lahko dodatno ugotovimo, da se je večina vprašanj prve kategorije nanašala na pravilnost in kakovost kode, pri drugi in tretji težavnostni kategoriji pa se je večina vprašanj nanašala na rešitev same naloge ter popravljanje nedelujoče kode.\\
...
Če učence razdelimo glede na njihovo starost, dobimo dve skupini: 14 učencev v starosti 11-13 let in 17 učencev v starosti 14-16 let. Graf \ref{fig:starostGPT} prikazuje število točk po kategorijah glede na uporabo ChatGPT-ja za obe starostni skupini. Pri nalogah prve kategorije ni velike razlike v reševanju med skupinama, razliko pa opazimo pri nalogah druge in tretje kategorije, kjer vidimo, da je starejša skupina boljše reševala naloge, ki jih je bilo potrebno reševati samostojno, kar lahko pripišemo več izkušnjam, pridobljenih s starostjo. Pri primerjavi nalog, rešenih z asistenco GPT, opazimo, da so povprečne ocene zelo podobne. Pri nalogah druge in tretje kategorije dosega mlajša skupina celo višje rezultate, kar dokazuje, da uporaba ChatGPT-ja omogoča reševanje tudi zahtevnejših nalog, ne glede na stopnjo predhodnega znanja ali izkušenj, in omogoča izenačevanje razlik med starostnimi skupinami.  \\
...
\subsection{Omejitve in implikacije}
 Pomembno je izpostaviti tudi nekatere omejitve raziskave. Vzorec udeležencev je bil omejen večinoma na moško populacijo istega tečaja programiranja, poleg tega je število nalog bilo majhno zaradi časovne omejitve eksperimenta, kar zmanjšuje reprezentativnost rezultatov. \\
Iz rezultatov eksperimenta lahko sklepamo, da programiranje z asistenco umetne inteligence izboljša učinkovitost, pravilnost programov in hitrost razvoja tako pri začetnikih kot tudi pri programerjih z več izkušnjami. Učinki uporabe umetne inteligence so bili najbolj izraziti pri zahtevnejših nalogah, kjer je bilo pogovorno okno bolj pogosto uporabljeno, medtem ko je bila pri lažjih nalogah njena vloga manj opazna, saj so učenci te naloge bolj pogosto reševali samostojno. Največjo prednost inženiring pozivov prinaša manj izkušenim programerjem, saj jim omogoča reševanje zahtevnejših nalog z dodatnimi razlagami in zmanjšuje razlike v primerjavi s programerji z več izkušnjami.
...
\chapter{Zaključek}
Diplomska naloga je raziskala potencial inženiringa pozivov, podprtega z velikimi jezikovnimi modeli, kot ključno orodje pri razvoju programske opreme. Študija je pokazala, da lahko orodja, kot je ChatGPT, bistveno izboljšajo učinkovitost programiranja, zmanjšajo število napak in pospešijo proces razvoja programske opreme. Analiza rezultatov je v povprečju pokazala 23~\% izboljšanje rezultatov pri reševanju nalog v primerjavi z metodami brez podpore UI. Največja razlika se je pokazala pri nalogah višjih težavnostnih kategorij, kjer so naloge z uporabo UI bile v povprečju rešene za 32~\% boljše.

Pomemben prispevek naloge je poudarek na zmanjševanju razkoraka med  razvijalci z različnimi stopnjami predznanja. Uporaba orodij UI ne le pospešuje proces kodiranja, temveč tudi omogoča programerjem z manj izkušnjami reševanje težjih izzivov.

Kljub obetavnim rezultatom pa raziskava opozarja na nekatere omejitve, kot so tveganja prekomernega zanašanja na UI ter potreba po natančnem preverjanju generirane kode. Te omejitve zahtevajo odgovorno uporabo orodij UI in nadaljnje raziskave za izboljšanje njihove zanesljivosti.

Rezultati naloge dokazujejo, da inženiring pozivov, podprt z umetno inteligenco, odpira nove možnosti za optimizacijo procesov razvoja programske opreme. Praktični dosežki študije lahko pripomorejo nadaljnjim raziskavam o širši uporabi teh orodij v različnih programerskih okoljih ter njihov vpliv na različne razvojne scenarije.

V nalogi ugotavljamo, da je integracija umetne inteligence v programsko inženirstvo pomemben korak k učinkovitejšemu, natančnejšemu in hitrejšemu razvoju programske opreme, ki bo igrala ključno vlogo v prihodnosti tehnologije.
